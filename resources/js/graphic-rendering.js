define(["jquery", 'moment', "./components/lang", "./dependencies/lagrange" /*, "anychart", "anychart-jquery"*/],
	function ($, moment, lang) {
		...
		function getRegressionCurve(data) {
			...
			let estimated_data = [];
			// [{
			// 	projectedAverage: tabResult[data.length - 1].average,
		...
		function onDataReceive() {
			...
			const tempsWithAverage = getRegressionCurve(data.temps);
			// const projectedDatas = getProjectedCurve(tempsWithAverage);
			// const tempsWithAll = tempsWithAverage.concat(projectedDatas);
			// add data
			...
			// var projectedAverage = plot.spline(
			// 	dataTable.mapAs({
			// 		value: "projectedAverage",
			// 		x: "created_at"
			// 	})
			// ).name("Provisional").stroke({ color: 'cyan', dash: "5 5" });
